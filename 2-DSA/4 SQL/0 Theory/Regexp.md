...
- Matches any character listed between the square brackets (`[abc]`): Gives all the names containing 'j' or 'z'. Example – Lorentz, Rajs.
    ...
    ```
- Matches any lower case letter between 'a' to 'z' (`[a-z]` and `.`): Retrieve all names that contain a letter in the range of 'b' and 'g', followed by any character, followed by the letter 'a'. Example – Tobias, sewall. Matches any single character (`.`)
    ```sql
