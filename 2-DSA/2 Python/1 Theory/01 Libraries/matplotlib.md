...
- **Plotly Express** :
    - Plotly Express is a part of the Plotly library and is used for creating interactive, high-level visualizations.
    - It provides a simplified and expressive interface for creating a wide range of charts and graphs.
    - Plotly is known for its interactive and web-based plotting capabilities.

- **Matplotlib** :
    - Matplotlib is a separate, popular data visualization library in Python.
    - It is a lower-level library that offers more fine-grained control over plot customization.
    - While Matplotlib can be used to create static plots, it can also be embedded in Jupyter Notebooks for interactive use.
...
2. `plot()` : function is used to draw points (markers) in a diagram.
    ...
    - **marker** :
        ...
        - both `mec` and `mfc` : `plt.plot(ypoints, marker = 'o', ms = 20, mec = 'r', mfc = 'r')`
    ...
    - To plot multiple lines on same graph:
...
6. `scatter()` : to draw scatter plot. The scatter() function plots one dot for each observation. It needs two arrays of the same length, one for the values of the x-axis, and one for values on the y-axis.
    ...
    - Color Each Dot : You can even set a specific color for each dot by using an array of colors as value for the c argument:
        ...
        ```
    - ColorMap : The Matplotlib module has a number of available color-maps. A colormap is like a list of colors, where each color has a value that ranges from 0 to 100.
...
7. `bar()` : to draw bar graphs
    ...
    - `width` : The `bar()` takes the keyword argument width to set the width of the bars
        ```python
